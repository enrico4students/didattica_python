import os
print(f"directory di lavoro del programma: {os.getcwd()}")
...
def test_ex1_2():
    ...
    out = {'ananab': (set(['banana']), 1), 'annaa': (
        set(['anana', 'naana']), 2), '2552': (set(), 0)}
    return do_ex1_tests(input, out)
...
def test_ex2_2():
    input = ['banana', 'ananab', 'anana', 'naana', 'annaa', '2552']
    out = {'annaa': (set(), 0), 'banana': (set(['anana']), 1), 'ananab': (
        set(['anana']), 1), 'naana': (set(), 0), '2552': (set(), 0)}
    return do_ex2_tests(input, out)
...
def test_ex3_2():
    input = ['banana', 'ananab', 'anana', 'naana', 'annaa', '2552']
    out = {True: (set(['anana', '2552']), 2), False: (
        set(['annaa', 'banana', 'ananab', 'naana']), 4)}
    return do_ex3_tests(input, out)
